...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="test">
        ...
        <xsl:variable name="testRefsValueOf">
            <xsl:value-of select="$testRefs"/>
        </xsl:variable>
        <xsl:variable name="outputRefsValueOf">
        ...
        <xsl:choose>
            <xsl:when test="following-sibling::*[1]=following-sibling::output[1]">
                <xsl:choose>
                    ...
                    <xsl:when test="(count($testRefs)=count($outputRefs)) and ($testRefsValueOf=$outputRefsValueOf)">
