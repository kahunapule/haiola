        ...
        <xsl:choose>
            <xsl:when test="$bookHasOnlyOneChapter=true()">
                <xsl:choose>
                    ...
                    <xsl:when test="$chapterNumber='1' and $verseNumber">
                        <!-- reference to XXX.1.1 in single chapter book-->
                        <xsl:element name="ref">
                            <xsl:attribute name="tgt">
                                <xsl:value-of select="$SILbookCode"/>
                                <xsl:text>.1</xsl:text>
...
<xsl:template match="v">
    <xsl:element name="v">
        ...
        <xsl:attribute name="bcv">
            <xsl:value-of select="ancestor::book[1]/@id"/>
            <xsl:text>.</xsl:text>
