...
<!-- history -->
<!-- 2015-05-08 previous xslt does not catch Psalm 18; and 29-32 gets all wrong so correct here -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="ref">
        ...
        <xsl:variable name="verseBridge" select="contains(.,'-')"/>
        <xsl:choose>
            <xsl:when test="verseBridge and parsedBCV[1]='previousBookCodeHere'">  <!-- and $parsedBCV[1]='previousBookCodeHere' and $parsedBCV[3]='1'"> -->
      <xsl:element name="ref">
         <!-- use  chapter number from preceding ref --> 
       <xsl:attribute name="jim">ssssssss</xsl:attribute>
          <xsl:attribute name="tgt">
              ...
              <xsl:text>.</xsl:text>
              <xsl:value-of select="parsedBCV[3]"/>    
          </xsl:attribute>
      </xsl:element>          
