...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="ref">
        ...
        <xsl:choose>
         ...
         <!--  If in intro we get unusual references we may need to turn this next test on which requires a preceding book code. 
     ...
     -->
       ...
       <!-- Persian and -->     
            <xsl:when test="$tokenizedPrecedingChapterTarget[2] and $lengthPrecedingText &lt; 8 and contains($precedingWord,'و') ">
            ...
            <!--  /and/ -->
            <xsl:when test="$tokenizedPrecedingChapterTarget[2] and $lengthPrecedingText &lt; 8 and contains($precedingWord,'and') and $tokenizedCurrentTarget[3]='1'">
                <!-- switch chapter and verse-->
                <xsl:element name="ref">
                    <xsl:attribute name="tgt">
                        <xsl:value-of select="$tokenizedCurrentTarget[1]"/>
